...
For multi-repository work, keep separate scoped commits. Every product delivery-slice PR must cross-reference its exact product and control commit SHAs and verification results. Control evidence may accumulate in one final verification PR when its independent test increments remain separately committed.

Opening a validated PR is the automation boundary. Do not merge a PR, publish or release an artifact, or delete its branch without a separate explicit Human decision.
