...
function queueDraftFollowUp(pi: ExtensionAPI, stage: string, path?: string): void {
  const replyContract = `Final user-facing reply requirements, in Chinese: start by saying you are their Plan assistant and what you just helped organize; show the generated section content in the reply so the user can review without opening the file; then show the saved file path; then explain both edit paths: they can tell you natural-language changes such as “把 X 加进去/范围缩小一点”, or manually edit the Markdown file and say “我改好了，检查一下”; finally state the next approval phrase. Do not mention internal tool names.`;
  const instruction = stage === "what_why"
    ? `Continue the current Harness Plan${path ? ` at ${path}` : ""}. Use plan_get to read the snapshot. Draft the What / Why from Original Request, including goal, why, constraints, boundaries, success criteria, and any implementation ideas as non-binding context. Use plan_submit_section to submit it. If information is missing, continue with explicit Open Questions instead of blocking. Stop after submission; do not advance to Plan. Next approval phrase: “继续”. ${replyContract}`
    : stage === "plan"
      ? `Continue the current Harness Plan${path ? ` at ${path}` : ""}. Use plan_get, draft the Plan from approved What / Why, and use plan_submit_section. Stop after submission; do not create Tasks. Next approval phrase: “继续” or “开始拆任务”. ${replyContract}`
      : `Continue the current Harness Plan${path ? ` at ${path}` : ""}. Use plan_get, draft current-round Tasks only from approved Plan T+0, and use plan_submit_section. Stop after submission; do not run Review. Next phrase: “检查一下这些任务”. ${replyContract}`;
