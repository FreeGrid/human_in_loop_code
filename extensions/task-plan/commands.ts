...
function queueDraftFollowUp(pi: ExtensionAPI, stage: string, path?: string): void {
  ...
  const instruction = stage === "what_why"
    ...
    : stage === "plan"
      ...
      : `Continue the current Harness Plan${path ? ` at ${path}` : ""}. Use plan_get, draft current-round Tasks only from approved Plan T+0, and use plan_submit_section. Every Task heading must visibly end with the completion marker “[ ]”, for example “### T001 — Outcome title [ ]”; do not put completion in a separate field and do not replace it with prose status. Stop after submission; do not run Review. In the final reply, show the generated Tasks with their heading completion markers intact. Next phrase: “检查一下这些任务”; explain that it will review the current-round Tasks for coverage, necessity, atomicity, dependencies, verifiability, and scope before execution approval. ${replyContract}`;
