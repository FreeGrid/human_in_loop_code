...
function queueDraftFollowUp(pi: ExtensionAPI, stage: string, path?: string): void {
  ...
  const instruction = stage === "what_why"
    ...
    : stage === "plan"
      ? `Continue the current Harness Plan${path ? ` at ${path}` : ""}. Use plan_get, draft the Plan from approved What / Why, and use plan_submit_section. The Plan must include T+0 and T+1; include T+2 when there is any meaningful later direction, but keep later horizons conditional. Stop after submission; do not create Tasks. Next phrase: “继续” or “开始拆任务”; explain that it will approve this Plan and start drafting current-round Tasks from T+0. ${replyContract}`
      : `Continue the current Harness Plan${path ? ` at ${path}` : ""}. Use plan_get, draft current-round Tasks only from approved Plan T+0, and use plan_submit_section. List stage Tasks directly as “### T001 — Stage title [ ]”, “### T002 — ... [ ]”; do not add a separate T+0 grouping block. Treat each Txxx as a larger stage/delivery unit and list smaller tasks under its “#### Tasks” subsection using trailing status markers like “- Smaller task [ ]”. Keep each stage concise with only Tasks, Acceptance, and Depends On subsections; do not add visible Round, Outcome, Why, Inputs, Work, or Outputs. If the current round is after R000, add only the hidden metadata line \`<!-- pi-plan:round:RNNN -->\` immediately below each new stage Task heading. Stop after submission; do not run Review. In the final reply, show the generated Tasks with their heading completion markers intact. Next phrase: “检查一下这些任务”; explain that it will review the current-round Tasks for coverage, necessity, atomicity, dependencies, verifiability, and scope before execution approval. ${replyContract}`;
  pi.sendMessage({ customType: "pi-plan-guided-draft", content: instruction, display: false }, { triggerTurn: true, deliverAs: "followUp" });
}
