...
export async function documentAuthorityContext(document: PlanDocument, node_id = "$plan", roots: { target_root?: string; governance_root?: string } = {}, runtime?: EvidenceRuntime): Promise<AuthorityContext> {
  ...
  return { plan_id: document.metadata.plan_id, node_id, document_hash: document.document_hash,
    contract_hash: node_id === "$plan" ? phaseExecutionDefinitionHash(document) : nodeContractHash(document, node_id, runtime), target_root, governance_root };
}

export async function consumeDocumentAuthority(document: PlanDocument, token: HumanCapability | undefined, action: AuthorityAction, node_id = "$plan", roots: { target_root?: string; governance_root?: string } = {}, runtime?: EvidenceRuntime): Promise<AuthorizationReceipt> {
  assertTransitionAllowed(document.metadata.stage, action);
  return consumeHumanCapability(token, action, await documentAuthorityContext(document, node_id, roots, runtime));
}
