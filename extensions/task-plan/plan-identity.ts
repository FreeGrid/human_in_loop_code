...
export function nodeDeclaredContract(document: PlanDocument, id: string) {
  ...
  return {...(node.requestedRole!==undefined?{requested_role:node.requestedRole}:{}),...(node.humanGates!==undefined?{human_gates:node.humanGates}:{}),schema:"pi-plan/contract/node-v1",plan_id:document.metadata.plan_id,node_id:id,outline:nodeDeclaredOutline(document,id),round:node.round,what_why:d.whatWhy,strategy:d.strategy,work:uncheck(node.work),acceptance:uncheck(node.acceptance),depends_on:[...node.dependsOn].sort(),verification:node.verification,scopes:node.scopes,forbidden:node.forbidden,non_goals:node.nonGoals,review_policy:node.reviewPolicy,...(node.legacyDefinition?{legacy_definition:node.legacyDefinition}:{})};
}
