...
`[x]` means currently considered complete. Humans may check a task directly, including in an external editor. Ordinary completion does not need authentication or evidence. Completing a top-level task changes only its own checkbox and preserves every subtask's text, order and individual checkbox. Reading, saving, reopening or selecting another task does not remove recorded children. A checked parent does not automatically check its children. Explicitly editing the current subtask list can still revise or remove items. Do not append results, summaries or verification details. A substantive requirement or task edit normally reopens affected work; the Human may mark it complete again.
