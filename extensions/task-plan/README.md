...
Plan 按需使用。临时查看文件、执行命令、启动子 Agent 或等待，可以直接让助手做，不必先建计划或切换阶段。即使已经选中一份 Plan，独立请求也不会仅因此被要求读写它；你明确要求规划，或上下文已把请求关联到当前 Plan 时，才进入下面的流程。自然语言路由依靠模型遵循提示，具体行为仍需核对实际工具调用。
...
在已请求的 Plan 工作流中，默认选中的 Plan 在完成后也继续有效。新增需求、改变范围甚至改变目标，都先更新当前文件；只有你明确说“新开一个 Plan”，或运行 `/plan:new`，才建立另一份。这样不会因为一句补充需求，在目录里生成两份互相竞争的“当前计划”。
