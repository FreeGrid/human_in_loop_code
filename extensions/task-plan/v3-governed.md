...
Verification failure, stale inputs, out-of-scope writes, invalid authority and missing review prevent governed completion. Finalize repeats readiness checks after confirmation, durably records acceptance and pending projection, then writes the checked task without a result summary. Ordinary `[x]` never synthesizes verified evidence. An observed material definition change retires old evidence even if the text is later restored.
